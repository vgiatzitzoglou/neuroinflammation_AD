%% MASTER SCRIPT: GENERATE ALL SSA FILES FROM SCRATCH (FIXED SCALING)
% This script runs the complete Single Subject Analysis pipeline.
%
% FIX V2: Uses GM Mask for Intensity Scaling (ignores background noise).
%
% OUTPUTS GENERATED:
...
% Load Mask for accurate Control Mean calculation
...
for i = 1:length(ctrl_subs)
    ...
    if ~isempty(f)
        try
            ...
            % Calc Mean INSIDE MASK only
            brain_vals = dat(mask_idx);
            brain_vals = brain_vals(brain_vals > 0); % Remove zeros/NaNs
...
for g = 1:length(groups)
    ...
    for p = 1:length(pat_subs)
        ...
        try
            ...
            fprintf('  [OK] Clusters.nii created\n');
        catch ME
...
function f_out = check_and_rescale(f_in, target, mask_p)
    ...
    % Load Mask to ensure we only measure brain
    Vm = spm_vol(mask_p);
    M = spm_read_vols(Vm);
    ...
    % Calculate Mean inside Mask
    brain_vals = Y(mask_idx);
    brain_vals = brain_vals(brain_vals > 0); % Safety check
    curr_mean = mean(brain_vals);
    ...
    if curr_mean < 1.0 
        ...
        V_out = V;
        V_out.fname = f_out;
        V_out.dt = [16 0]; % FORCE FLOAT32 to avoid clipping
